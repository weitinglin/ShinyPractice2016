...
PO <- read.csv(inFile$datapath, stringsAsFactors = FALSE)
...
PO$Area[grep("710i", PO$HealthServices, ignore.case = TRUE)] <- "Elephant"
...
#produce a data column from the data column in the spreadsheet

PO$Date <- as.Date(substr(PO$dtSubmitted, 1, 10), format = "%Y-%m-%d")

#produce a date column from the data column in the 


   
   #select only the area as selectyed in the UI 
   ...
   output$outputLink <- renderText({
     ...
     link <- switch(input$area, "Armadillo" = "http://www.patientopinion.org.uk/services/rhary",
     ...
     paste0('form action = "', link, '"target = "_blank"><input type="submit" value="Go to main site"></form>')
   })
 })